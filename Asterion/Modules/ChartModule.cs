...
public class ChartModule : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public class ChartType : ChartModule
    {
        ...
        [SlashCommand("30d", "[Experimental] Graphs the downloads of a project over the span of 30 days", runMode: RunMode.Async)]
        public async Task Daily([Autocomplete(typeof(SubscribedIdAutocompletionHandler))] string projectSlugOrId)
        {
            ...
            {
                _logger.LogDebug("Not enough data to create a graph, we have {DownloadDataCount} data points", downloadData.Count);
                await FollowupAsync("Not enough data to create a graph, please wait for a bit (~5 days)", ephemeral: true);
                return;
            }
            ...
            await FollowupWithFileAsync(file, "Downloads of " + project.Payload.Project.Title + " over the last 30 days");
        }
